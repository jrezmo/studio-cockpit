...
/**
 * @file
 * @brief ptslcmd.cpp wrapper for the PTSLC_CPP::CppPTSLClient::Redo command.
 */
...
const string g_pszRedo = "Redo";
const string g_pszRedoHelp = "Redo -levels <levels>";

PtslCmdCommandResult Redo(const vector<string>& params, CppPTSLClient& client)
{
    // Create a request:
    RedoRequest request;
    request.commandType = CommandType::Redo;
    request.levels = 0;
    ...
    std::shared_ptr<RedoResponse> rsp = client.Redo(request);
    ...
    {
        cout << "Redo returned an unexpected status" << endl;
    }
