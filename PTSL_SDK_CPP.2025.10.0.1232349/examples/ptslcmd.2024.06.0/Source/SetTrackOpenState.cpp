...
/**
 * @file
 * @brief ptslcmd.cpp wrapper for the PTSLC_CPP::CppPTSLClient::SetTrackOpenState command.
 */

#include "Common.h"

const string g_pszSetTrackOpenState = "SetTrackOpenState";
const string g_pszSetTrackOpenStateHelp =
    "SetTrackOpenState -track_names <track_name> [<track_name> ...] -enabled <true|false>";

PtslCmdCommandResult SetTrackOpenState(const vector<string>& params, CppPTSLClient& client)
{
    // Create a request:
    SetTrackOpenStateRequest request;
    request.commandType = CommandType::SetTrackOpenState;
    ...
    std::shared_ptr<CommandResponse> rsp = client.SetTrackOpenState(request);
