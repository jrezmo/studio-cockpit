...
/**
 * @file
 * @brief ptslcmd.cpp wrapper for the PTSLC_CPP::CppPTSLClient::SetTrackDSPModeSafeState command.
 */

#include "Common.h"

const string g_pszSetTrackDSPModeSafeState = "SetTrackDSPModeSafeState";
const string g_pszSetTrackDSPModeSafeStateHelp =
    "SetTrackDSPModeSafeState -track_names <track_name> [<track_name> ...] -enabled <true|false>";

PtslCmdCommandResult SetTrackDSPModeSafeState(const vector<string>& params, CppPTSLClient& client)
{
    // Create a request:
    SetTrackDSPModeSafeStateRequest request;
    request.commandType = CommandType::SetTrackDSPModeSafeState;
    ...
    std::shared_ptr<CommandResponse> rsp = client.SetTrackDSPModeSafeState(request);
