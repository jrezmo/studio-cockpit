...
/**
 * @file
 * @brief ptslcmd.cpp wrapper for the PTSLC_CPP::CppPTSLClient::SetTrackSoloSafeState command.
 */

#include "Common.h"

const string g_pszSetTrackSoloSafeState = "SetTrackSoloSafeState";
const string g_pszSetTrackSoloSafeStateHelp =
    "SetTrackSoloSafeState -track_names <track_name> [<track_name> ...] -enabled <true|false>";

PtslCmdCommandResult SetTrackSoloSafeState(const vector<string>& params, CppPTSLClient& client)
{
    // Create a request:
    SetTrackSoloSafeStateRequest request;
    request.commandType = CommandType::SetTrackSoloSafeState;
    ...
    std::shared_ptr<CommandResponse> rsp = client.SetTrackSoloSafeState(request);
