...
/**
 * @file
 * @brief ptslcmd.cpp wrapper for the PTSLC_CPP::CppPTSLClient::SetTrackSoloState command.
 */

#include "Common.h"

const string g_pszSetTrackSoloState = "SetTrackSoloState";
const string g_pszSetTrackSoloStateHelp =
    "SetTrackSoloState -track_names <track_name> [<track_name> ...] -enabled <true|false>";

PtslCmdCommandResult SetTrackSoloState(const vector<string>& params, CppPTSLClient& client)
{
    // Create a request:
    SetTrackSoloStateRequest request;
    request.commandType = CommandType::SetTrackSoloState;
    ...
    std::shared_ptr<CommandResponse> rsp = client.SetTrackSoloState(request);
