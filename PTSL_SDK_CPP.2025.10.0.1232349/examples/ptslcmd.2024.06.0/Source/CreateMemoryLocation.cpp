...
/**
 * @file
 * @brief ptslcmd.cpp wrapper for the PTSLC_CPP::CppPTSLClient::CreateMemoryLocation command.
 */

#include "Common.h"
const string g_pszCreateMemoryLocation = "CreateMemoryLocation";
const string g_pszCreateMemoryLocationHelp =
    "CreateMemoryLocation [-number <number>] [-name <name>] [-start_time <start_time>] [-end_time <end_time>] [-time_properties {TP_Marker|TP_Selection|TP_None}] [-reference {MLR_BarBeat|MLR_Absolute|MLR_FollowTrackTimebase}] [-general_properties <0/1 flags: zoom_settings pre_post_roll_times track_visibility track_heights group_enables window_configuration e.g. 1 0 0 1 1 0>] [-window_configuration_index <window_configuration_index>] [-comments <comments>] [-color_index <color>] [-location {MLC_MainRuler|MLC_Track}] [-track_name <track_name>] [-venue_snapshot_index <venue_snapshot_index>] [-venue_snapshot_name <venue_snapshot_name>]";

PtslCmdCommandResult CreateMemoryLocation(const vector<string>& params, CppPTSLClient& client)
{
    // Create a request:
    CreateMemoryLocationRequest request;
    request.commandType = CommandType::CreateMemoryLocation;
    ...
    // then parse `params` and fill the request manually:
    {
        ...
        request.startTime = "";
        request.endTime = "";
        request.timeProperties = TimeProperties::TP_Marker;
        request.reference = MemoryLocationReference::MLR_Absolute;
        request.generalProperties.zoomSettings = false;
        request.generalProperties.prePostRollTimes = false;
        request.generalProperties.trackVisibility = false;
        request.generalProperties.trackHeights = false;
        request.generalProperties.groupEnables = false;
        request.generalProperties.windowConfiguration = false;
        request.generalProperties.windowConfigurationIndex = 0;
    ...
    std::shared_ptr<CommandResponse> rsp = client.CreateMemoryLocation(request);
