...
/**
 * @file
 * @brief ptslcmd.cpp wrapper for the PTSLC_CPP::CppPTSLClient::SetTrackInputMonitorState command.
 */

#include "Common.h"

const string g_pszSetTrackInputMonitorState = "SetTrackInputMonitorState";
const string g_pszSetTrackInputMonitorStateHelp =
    "SetTrackInputMonitorState -track_names <track_name> [<track_name> ...] -enabled <true|false>";

PtslCmdCommandResult SetTrackInputMonitorState(const vector<string>& params, CppPTSLClient& client)
{
    // Create a request:
    SetTrackInputMonitorStateRequest request;
    request.commandType = CommandType::SetTrackInputMonitorState;
    ...
    std::shared_ptr<CommandResponse> rsp = client.SetTrackInputMonitorState(request);
