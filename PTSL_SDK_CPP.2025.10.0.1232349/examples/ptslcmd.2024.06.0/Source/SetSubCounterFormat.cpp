...
/**
 * @file
 * @brief ptslcmd.cpp wrapper for the PTSLC_CPP::CppPTSLClient::SetSubCounterFormat command.
 */

#include "Common.h"

const string g_pszSetSubCounterFormat = "SetSubCounterFormat";
const string g_pszSetSubCounterFormatHelp =
    "SetSubCounterFormat [-time_scale {BarsBeats|MinSecs|TimeCode|FeetFrames|Samples}]";

PtslCmdCommandResult SetSubCounterFormat(const vector<string>& params, CppPTSLClient& client)
{
    // Create a request:
    SetSubCounterFormatRequest request;
    request.commandType = CommandType::SetSubCounterFormat;
    ...
    auto rsp = client.SetSubCounterFormat(request);
