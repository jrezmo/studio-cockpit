...
/**
 * @file
 * @brief ptslcmd.cpp wrapper for the PTSLC_CPP::CppPTSLClient::SetTrackFrozenState command.
 */

#include "Common.h"

const string g_pszSetTrackFrozenState = "SetTrackFrozenState";
const string g_pszSetTrackFrozenStateHelp =
    "SetTrackFrozenState -track_names <track_name> [<track_name> ...] -enabled <true|false>";

PtslCmdCommandResult SetTrackFrozenState(const vector<string>& params, CppPTSLClient& client)
{
    // Create a request:
    SetTrackFrozenStateRequest request;
    request.commandType = CommandType::SetTrackFrozenState;
    ...
    std::shared_ptr<CommandResponse> rsp = client.SetTrackFrozenState(request);
