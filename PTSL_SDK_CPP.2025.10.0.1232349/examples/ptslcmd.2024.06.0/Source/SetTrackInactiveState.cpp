...
/**
 * @file
 * @brief ptslcmd.cpp wrapper for the PTSLC_CPP::CppPTSLClient::SetTrackInactiveState command.
 */

#include "Common.h"

const string g_pszSetTrackInactiveState = "SetTrackInactiveState";
const string g_pszSetTrackInactiveStateHelp =
    "SetTrackInactiveState -track_names <track_name> [<track_name> ...] -enabled <true|false>";

PtslCmdCommandResult SetTrackInactiveState(const vector<string>& params, CppPTSLClient& client)
{
    // Create a request:
    SetTrackInactiveStateRequest request;
    request.commandType = CommandType::SetTrackInactiveState;
    ...
    std::shared_ptr<CommandResponse> rsp = client.SetTrackInactiveState(request);
