...
/**
 * @file
 * @brief ptslcmd.cpp wrapper for the PTSLC_CPP::CppPTSLClient::SetTrackMuteState command.
 */

#include "Common.h"

const string g_pszSetTrackMuteState = "SetTrackMuteState";
const string g_pszSetTrackMuteStateHelp =
    "SetTrackMuteState -track_names <track_name> [<track_name> ...] -enabled <true|false>";

PtslCmdCommandResult SetTrackMuteState(const vector<string>& params, CppPTSLClient& client)
{
    // Create a request:
    SetTrackMuteStateRequest request;
    request.commandType = CommandType::SetTrackMuteState;
    ...
    std::shared_ptr<CommandResponse> rsp = client.SetTrackMuteState(request);
