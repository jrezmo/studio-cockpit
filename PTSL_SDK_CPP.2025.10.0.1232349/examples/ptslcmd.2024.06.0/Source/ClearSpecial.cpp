...
/**
 * @file
 * @brief ptslcmd.cpp wrapper for the PTSLC_CPP::CppPTSLClient::ClearSpecial command.
 */

#include "Common.h"

const std::string g_pszClearSpecial = "ClearSpecial";
const std::string g_pszClearSpecialHelp =
    "ClearSpecial [-automation_data_option {All_Automation|Pan_Automation|PlugIn_Automation|Clip_Gain|Clip_Effects}]";

PtslCmdCommandResult ClearSpecial(const std::vector<std::string>& params, CppPTSLClient& client)
{
    ClearSpecialRequest request;
    request.commandType = CommandType::ClearSpecial;
    ...
    std::shared_ptr<CommandResponse> rsp = client.ClearSpecial(request);
