...
/**
 * @file
 * @brief ptslcmd.cpp wrapper for the PTSLC_CPP::CppPTSLClient::SetTrackHiddenState command.
 */

#include "Common.h"

const string g_pszSetTrackHiddenState = "SetTrackHiddenState";
const string g_pszSetTrackHiddenStateHelp =
    "SetTrackHiddenState -track_names <track_name> [<track_name> ...] -enabled <true|false>";

PtslCmdCommandResult SetTrackHiddenState(const vector<string>& params, CppPTSLClient& client)
{
    // Create a request:
    SetTrackHiddenStateRequest request;
    request.commandType = CommandType::SetTrackHiddenState;
    ...
    std::shared_ptr<CommandResponse> rsp = client.SetTrackHiddenState(request);
